...
bool HttpPost(const string path, const string payload, string &response_text)
{
   ...
   if(copied <= 1 || ArrayResize(body, copied - 1) != copied - 1)
   {
      const int encoding_error = GetLastError();
      PrintFormat("EMERALD request encoding failed path=%s terminal_error=%d", path, encoding_error);
      return false;
   }
   ...
   // Capture immediately: converting an empty response can overwrite _LastError.
   const int request_error = GetLastError();
   const ulong elapsed_ms = GetTickCount64() - started_ms;
   const int response_bytes = ArraySize(response);
   if(response_bytes > 0)
      response_text = CharArrayToString(response, 0, response_bytes, CP_UTF8);
   if(status_code >= 200 && status_code < 300)
      return true;
   const string category = (status_code >= 100 && status_code <= 599) ? "HTTP" : "TRANSPORT";
   const string preview = (response_bytes > 0) ? StringSubstr(response_text, 0, 256) : "<empty>";
   PrintFormat("EMERALD request failed path=%s category=%s status=%d terminal_error=%d elapsed_ms=%I64u request_bytes=%d response_bytes=%d response=%s",
               path, category, status_code, request_error, elapsed_ms,
               ArraySize(body), response_bytes, preview);
   return false;
}
...
// Tick epochs use broker wall time. Normalize to UTC before appending a Z suffix.
// TimeGMT depends on the Windows clock; backend independently rejects future clocks.
bool BrokerUtcOffsetSeconds(long &offset_seconds)
{
   if(MQLInfoInteger(MQL_TESTER) || !TerminalInfoInteger(TERMINAL_CONNECTED))
      return false;
   const long delta = (long)TimeTradeServer() - (long)TimeGMT();
   offset_seconds = (long)MathRound((double)delta / 900.0) * 900;
   if(MathAbs((double)(delta - offset_seconds)) > 30.0 ||
      MathAbs((double)offset_seconds) > 14.0 * 3600.0)
   {
      Print("EMERALD invalid broker/UTC offset; synchronize Windows clock");
      return false;
   }
   return true;
}
...
bool SendNewTicks()
{
   long broker_offset_seconds = 0;
   if(!BrokerUtcOffsetSeconds(broker_offset_seconds))
      return false;
   MqlTick ticks[];
   ...
   const string observed_at = IsoUtcMillis((long)TimeGMT() * 1000);
   string items = "";
   ...
   for(int index = 0; index < copied; ++index)
   {
      ...
      if(valid_count > 0)
         items += ",";
      const long utc_tick_msc = ticks[index].time_msc - broker_offset_seconds * 1000;
      const string timestamp = IsoUtcMillis(utc_tick_msc);
      items += StringFormat(
         "{\"symbol\":\"%s\",\"bid\":%.8f,\"ask\":%.8f,"
         "\"broker_time\":\"%s\",\"observed_at\":\"%s\",\"volume\":%.2f}",
         JsonEscape(_Symbol), ticks[index].bid, ticks[index].ask,
         timestamp, observed_at, (double)ticks[index].volume_real
      );
   ...
   const string payload = StringFormat(
      "{\"broker_id\":\"%s\",\"point\":%.10f,"
      "\"timestamp_contract\":\"utc-v1\",\"broker_utc_offset_seconds\":%I64d,\"ticks\":[%s]}",
      JsonEscape(BrokerId()), SymbolInfoDouble(_Symbol, SYMBOL_POINT), broker_offset_seconds, items
   );
   string response;
...
int OnInit()
{
   ...
   Print("RIRI EMERALD DEMO v1.204 initialized; production telemetry enabled, entries locked");
